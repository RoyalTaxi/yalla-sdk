...
public data class AnalyticsEvent(val name: String, val params: Map<String, ParamValue> = emptyMap())

public sealed interface ParamValue {
    public data class Text(val value: String) : ParamValue
    public data class Count(val value: Long) : ParamValue
    public data class Amount(val value: Double) : ParamValue
    public data class Flag(val value: Boolean) : ParamValue
}
...
public fun event(name: String, params: ParamScope.() -> Unit = {}): AnalyticsEvent = AnalyticsEvent(name, ParamScope().apply(params).params)
