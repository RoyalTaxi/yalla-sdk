...
- **Result & errors** — [`Either`][uz.yalla.core.result.Either] (the universal `Either<DomainError, T>`
...
- **Phone** — [`UzPhone`][uz.yalla.core.phone.UzPhone], the single source of truth for `+998` numbers
...
- **Typed ids** — [`Ids`][uz.yalla.core.identity.OrderId] value classes (type-safe on JVM/Android; note
  they erase to the underlying type at the ObjC/Swift boundary).
- **Preferences & session contracts** — the `*Preferences` interfaces (implemented in `datastore`) and
...
- **Decoders:** `from(id)` is total and normalizes case/whitespace via the internal `normalizedId()`
  helper, defaulting to a defined fallback; `of(...)` parsers are strict and may fail (return `null`).
...
- **PII safety:** credential/PII-bearing types ([`Client`][uz.yalla.core.profile.Client], `Order.Driver`,
