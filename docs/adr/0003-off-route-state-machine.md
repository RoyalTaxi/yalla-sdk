...
**Binary state, with hysteresis, an edge-latched signal, and a connector line.** No graded states.
...
| ON_ROUTE   | crossTrack > `offRouteEnterMeters`                   | OFF_ROUTE  | **latch one signal** (cooldown-gated); chord |
...
- **Edge latch + cooldown.** Only the ON_ROUTE→OFF_ROUTE *edge* latches a signal
  (`consumeOffRouteSignal()` returns it once), and only if `refetchCooldownMillis` has elapsed since
  the last latch. Staying off-route does not re-fire it. This is the refetch-storm guard.
...
- **Thresholds are named commonMain config** ([`RouteFollowingConfig`]): `routeSnapThreshold`,
  `routeArrivalThreshold`, `connectorHideThreshold` replace the one overloaded `30 m` literal, so a
  distance means the same thing on Android and iOS.
...
- ON_ROUTE→OFF_ROUTE (latch + chord): `offRouteGpsFallsBackToChordAndSurfacesOffRouteState`,
  `offRouteSignalIsEdgeLatchedNotPerFrame`.
- OFF_ROUTE stickiness (hysteresis) + OFF_ROUTE→ON_ROUTE: `offRouteUsesEnterExitHysteresis`.
- Signal cooldown: `offRouteSignalRespectsRefetchCooldown`.
...
- The car can no longer silently lie about being on the route; off-route is visible (state +
  connector) and actionable (one cooled-down signal).
...
- Two thresholds + cooldown are tuning knobs that need on-device calibration (default 30/15,
  cooldown 15 s). If on-device proves binary insufficient, the fork can be revisited toward graded
  states — this ADR would then be superseded, not amended.
